 ...
 * require Model
 */
const _model = require("../models/ivrHistoriesModel");
...
/**
 * API lưu lịch sử click IVR của khách hàng
 */
exports.create = async (req, res, next) => {
  try {
    ...
    let body = req.body;
    // console.log(body);
    let keyRequires = FIELD_IVR_HISTORIES.require;
    let keyCheckEXISTS = FIELD_IVR_HISTORIES.checkExists;
    _logger.log("info", "create, body IVR " + JSON.stringify(body));
    ...
    for (let index = 0; index < keyRequires.length; index++) {
      ...
      if (body[element] === undefined || body[element] === null) {
        return next(
          new ResError(
            ERR_400.code,
            `${ERR_400.message_detail.missingKey} ${FIELD_IVR_HISTORIES.getName[element]}`
          ),
          req,
          res,
          next
    ...
    _logger.log("info", "start, create IVR " + JSON.stringify(body));
    const doc = await _model.create(db, dbMssql, body);
    if (!doc)
      return next(new ResError(ERR_404.code, ERR_404.message), req, res, next);
    _logger.log("info", "end, create IVR " + doc.insertedId);
